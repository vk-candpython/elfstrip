...
/*-----------------------------------------------------------------------------*/
    ...
    */
    ...
    /* Identify targets where e_flags
       must be preserved (ARM and RISC-V) */
    kep_eflg =
        (e_machine == EM_ARM)
#if defined(EM_RISCV)
    ||  (e_machine == EM_RISCV)
    ...
    */
    ...
    /* Absolute address of the original source program header entry */
        ...
        /* Sanitize and compact
           the dynamic linking table */
            ...
            for (
                ...
                /* Drop debugger footprints completely */
                if (tg == DT_DEBUG) continue;


                /* Cut SONAME from executables/PIE targets */
                else if ((tg == DT_SONAME) && (
                    (e_type == ET_EXEC) || has_interp
                )) continue;


                /* Aggressive zero-value purge
                   with vital presence-tag safeguards */
                else if (!dvl &&
                    (tg != DT_NULL    ) &&
                    (tg != DT_TEXTREL ) &&
                    (tg != DT_BIND_NOW)
        ...
        /* Precompute trailing memory boundary */
            ...
            size_t   orig_sge = p_offset + p_filesz; // Hard ceiling (original segment end)
            ...
            /* 8-byte align segment */
            sge = (p_offset + (p_filesz + 7)) & ~(size_t)7;

            /* Prevent expanding beyond original size */
            if (sge > orig_sge) sge = orig_sge;

            /* Update physical segment size */
            p_filesz = sge - p_offset;
        }
        ...
        /* Calculate the absolute memory address
           of the target program header entry */
            ...
            /* Re-bind header descriptor
               to the new destination memory address */
            PHDR_DEF(ph, dst, is_32bit);
    ...
    size_t pct = ((uint64_t)(fsz - nwfsz) * 1000) / fsz;
