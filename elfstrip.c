...
/*-----------------------------------------------------------------------------*/
    ...
    */
    ...
    size_t ehdsz = is_32bit? // Architecture-specific ELF header size
        sizeof(Elf32_Ehdr) : sizeof(Elf64_Ehdr),
           dynsz = is_32bit? // Architecture-specific dynamic entry size
    ...
    /* Absolute address of the original source program header entry */
        ...
        /* Precompute trailing memory boundary
           and initialize byte accumulator */
            ...
            while ((p_filesz > ehdsz) && !*(seg_tail - 1)) {
            ...
            /* Update segment end
               if we trimmed anything */
            if (bzero > ZERO)
                sge = p_offset + p_filesz; // Recalculate exact segment upper boundary offset
        }
