    ...
    */
    ...
    for (uint16_t i = 0;  i < e_phnum;  ++i) {
        ...
        if (p_type == PT_INTERP) {
            ...
            /* Zero out trailing bytes after the null-terminated path string */
            if (ln < p_filesz) memset(pt + ln, ZERO, p_filesz - ln); 
        }
        ...
        if (p_type == PT_DYNAMIC) {
            ...
            for (
                ...
                /* Drop empty or debug entries */
                if ((tg == DT_DEBUG) || ((tg != DT_NULL) && !dvl)) 
                    continue;
        ...
        if (is_crit_seg(p_type)) {
            ...
            /* Zero out alignment constraints for non-loadable segments */
            if (p_type != PT_LOAD) HD_SET(phz, is_x86, p_align, ZERO);
