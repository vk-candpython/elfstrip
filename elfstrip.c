...
#define DYN_GET(dp, is_x86, field) \
    ((is_x86)? ((Elf32_Dyn*)(dp))->field : ((Elf64_Dyn*)(dp))->field)


/* Safely updates a field in the architecture-agnostic dynamic entry */
#define DYN_SET(dp, is_x86, field, val) do {          \
    if (is_x86) ((Elf32_Dyn*)(dp))->field = (val);    \
...
static inline void putdec(size_t num) {
    char buf[24];   // Digit buffer
    ...
    write(STDOUT_FILENO, &buf[i + 1], 22 - i);
}
    ...
    */
    ...
    for (uint16_t i = 0;  i < e_phnum;  ++i) {
        ...
        size_t   p_filesz = HD_GET(ph, is_x86, p_filesz), // Segment size within the file image
                 p_memsz  = HD_GET(ph, is_x86, p_memsz ), // Segment image size in memory
        ...
        switch (p_type) {
            case PT_NOTE         : // Zero out and Delete metadata and build notes
        ...
        if (p_type == PT_INTERP) {
            has_interp = 1; // Mark as executable (has PT_INTERP)
        ...
        /* Sanitize and compact the dynamic linking table */
            ...
            for (
                ...
                int64_t  tg  = DYN_GET(s, is_x86, d_tag     ); // Get dynamic tag
                uint64_t dvl = DYN_GET(s, is_x86, d_un.d_val); // Get dynamic value
                ...
                switch (tg) {
                    ...
                    case DT_FLAGS_1  : // Dynamic flags (aux): strip loader/path baggage
                        ...
                        ); if (dvl) DYN_SET(s, is_x86, d_un.d_val, dvl);
                        /* fallthrough */
        ...
        if (is_crit_seg(p_type)) {
            ...
            HD_SET(ph, is_x86, p_filesz, p_filesz);

            /* Sync memory size to file size
               while preserving PT_LOAD BSS allocations */
            if (p_type == PT_LOAD)
                HD_SET(ph, is_x86, p_memsz, (p_memsz > p_filesz)? p_memsz : p_filesz);
            else
                HD_SET(ph, is_x86, p_memsz, p_filesz);
    ...
    write(STDOUT_FILENO, "%)\n", 3);
    /* Example: [+] Stripped: 8520 -> 6704 bytes (-21.3%) */
