...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    regs->start = 0;   // Anchor the base region at the absolute start of the file (ELF Header)
    regs->end   = phe; // Mark the entire Program Header Table boundary as protected
    ...
    for (uint16_t i = 0;  i < e_phnum;  ++i) {
        ...
        if (p_type == PT_INTERP) {
            ...
            size_t ln = strnlen(pt, p_filesz) + 1;

            /* Zero out trailing bytes after 
               the null-terminated path string */
            if (ln < p_filesz) {
                memset(pt + ln, ZERO, p_filesz - ln);

                HD_SET(ph, is_x86, p_filesz, ln);
                HD_SET(ph, is_x86, p_memsz,  ln);
        ...
        if (p_type == PT_DYNAMIC) {
            ...
            if (wofs < p_filesz) {
                ...
                memset(dptr + wofs, ZERO, p_filesz - wofs);
        ...
        if (is_crit_seg(p_type)) {
            ...
            /* Sync compacted dynamic segment dimensions 
               to the output image */
            if (p_type == PT_DYNAMIC) {
                HD_SET(phz, is_x86, p_filesz, p_filesz);
                HD_SET(phz, is_x86, p_memsz,  p_filesz);
            }

            /* Zero out physical address for segment */
            HD_SET(phz, is_x86, p_paddr, ZERO);
    ...
    for (uint16_t i = 0;  i < aphnm;  ++i) {
        ...
        if (HD_GET(phz, is_x86, p_type) == PT_PHDR) {
            ...
            HD_SET(phz, is_x86, p_filesz, nwsz);
            HD_SET(phz, is_x86, p_memsz,  nwsz);
            break;
        }
    } }
